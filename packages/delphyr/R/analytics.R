...
#' @examples
#' classify_consensus(
#'   list(n_valid = 10, n_agree = 7, n_disagree = 1),
#'   demo_protocol()$analysis$consensus
#' )
classify_consensus <- function(counts, rule) {
  ...
  pa <- if (n > 0) a / n else NA_real_
  pd <- if (n > 0) d / n else NA_real_
  status <- if (n < rule$min_valid_n) {
    "insufficient_data"
  } else if (rule_matches(pa, pd, rule$`in`)) {
    "consensus_in"
  } else if (rule_matches(pa, pd, rule$out)) "consensus_out" else "no_consensus"
...
new_snapshot <- function(responses, assignments, items, protocol, round_number = 1L, snapshot_id = NULL) {
  ...
  identifiers <- function(x) {
    is.character(x) && !anyNA(x) &&
      all(nzchar(trimws(x)) & !grepl("[\\x01-\\x1f]", x, perl = TRUE))
  }
  ensure(is.data.frame(assignments) && all(c("panelist_id", "group_code", "submitted") %in% names(assignments)), "assignments")
  ensure(identifiers(assignments$panelist_id) && identifiers(assignments$group_code), "assignments.identifiers")
  ...
  ensure(all(vapply(items[c("item_code", "dimension_code", "scale_code")], identifiers, logical(1))), "items.identifiers")
  ensure(!anyDuplicated(items[c("item_code", "dimension_code")]), "items.multiple_versions")
  ...
  ensure(all(vapply(responses[c("panelist_id", "item_code", "dimension_code")], identifiers, logical(1))) &&
    !anyNA(responses$answer_status) && is.character(responses$answer_status), "responses.identifiers_status")
  ensure(!anyDuplicated(paste(responses$panelist_id, key(responses), sep = "\034")), "responses.duplicate")
  ...
  assignments <- assignments[, c("panelist_id", "group_code", "submitted"), drop = FALSE]
  items <- items[, c("item_code", "item_version", "dimension_code", "scale_code"), drop = FALSE]
...
demo_snapshot <- function(values = c(1, 4, 6, 7, 7, 8, 8, 9, 9, 9), groups = rep("professionals", length(values))) {
  ...
  r <- data.frame(
    panelist_id = a$panelist_id, item_code = "I001", item_version = 1L, dimension_code = "relevance",
...
analyse_round <- function(snapshot, rules = snapshot$protocol$analysis$consensus) {
  ...
  ensure(identical(snapshot$content_hash, content_hash(list(
    data = snapshot$data, items = snapshot$items,
    protocol = snapshot$protocol, round_number = snapshot$round_number
  ))), "snapshot.hash")
  validate_rule(rules)
  ...
  for (j in seq_len(nrow(snapshot$items))) {
    it <- snapshot$items[j, , drop = FALSE]
    sc <- scales[[it$scale_code]]
    ...
    for (g in strata) {
      ...
      if (length(sc$levels)) {
        distribution[[length(distribution) + 1L]] <- data.frame(it,
          stratum = g,
          value = sc$levels, n = vapply(sc$levels, function(v) sum(x == v), integer(1)), row.names = NULL
        )
      }
    }
  }
  ...
  structure(list(
    results = res, decisions = decisions, distributions = dist,
...
compare_rounds <- function(previous, current, mapping = NULL) {
  analyse_round(previous)
  analyse_round(current)
  if (!is.null(mapping)) {
    ensure(is.data.frame(mapping) && all(c(
      "item_code", "dimension_code", "previous_version",
      "current_version", "comparable", "reason"
    ) %in% names(mapping)), "mapping")
    ensure(!anyNA(mapping) && is.logical(mapping$comparable) &&
      whole(mapping$previous_version) && whole(mapping$current_version) &&
      all(mapping$previous_version > 0 & mapping$current_version > 0) &&
      is.character(mapping$reason) && all(nzchar(trimws(mapping$reason))) &&
      !anyDuplicated(mapping[c("item_code", "dimension_code")]), "mapping")
  }
  out <- list()
  for (i in seq_len(nrow(current$items))) {
    it <- current$items[i, ]
    old <- previous$items[previous$items$item_code == it$item_code & previous$items$dimension_code == it$dimension_code, , drop = FALSE]
    ...
    subset_valid <- function(s) {
      z <- s$data
      z[z$item_code == it$item_code & z$dimension_code == it$dimension_code & z$submitted & z$answer_status == "answered" & !is.na(z$value_integer), c("panelist_id", "value_integer"), drop = FALSE]
    }
    a <- subset_valid(previous)
    b <- subset_valid(current)
    pairs <- merge(a, b, by = "panelist_id")
...
#' @export
print.delphyr_analysis <- function(x, ...) {
  print(x$decisions, row.names = FALSE)
  invisible(x)
}
#' @export
