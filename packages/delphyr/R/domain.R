...
new_scale <- function(type, levels = integer(), labels = as.character(levels),
  ...
  if (type == "free_text") {
    ensure(length(levels) == 0L, "scale.levels")
  } else {
  ...
  structure(list(
    type = type, levels = as.integer(levels), labels = labels,
...
validate_rule <- function(rule, scale = NULL) {
  ...
  a <- unlist(rule$agree_values)
  d <- unlist(rule$disagree_values)
  ...
  for (side in c("in", "out")) {
    ...
    for (v in c("agree", "disagree")) {
      ...
      known_keys(b, c("operator", "proportion"), "consensus.bound")
        ...
        b$operator %in% c("gte", "gt", "lte", "lt") && length(b$operator) == 1L,
        "consensus.operator"
      )
  ...
  # Intersect the two rectangular rule regions with a + d <= 1.
  # A finite sampling grid misses arbitrarily narrow strict-boundary overlaps.
  interval <- function(axis) {
    lo <- 0
    hi <- 1
    ...
    for (side in c("in", "out")) {
      b <- rule[[side]][[axis]]
      v <- b$proportion
      if (b$operator %in% c("gte", "gt")) {
        closed <- b$operator == "gte"
        if (v > lo) {
          lo <- v
          lo_closed <- closed
        } else if (v == lo) lo_closed <- lo_closed && closed
      } else {
        closed <- b$operator == "lte"
        if (v < hi) {
          hi <- v
          hi_closed <- closed
        } else if (v == hi) hi_closed <- hi_closed && closed
      }
    }
    list(
      lo = lo, closed = lo_closed,
      nonempty = lo < hi || (lo == hi && lo_closed && hi_closed)
    )
  }
  ai <- interval("agree")
  di <- interval("disagree")
  overlap <- ai$nonempty && di$nonempty &&
    (ai$lo + di$lo < 1 || (ai$lo + di$lo == 1 && ai$closed && di$closed))
...
demo_protocol <- function() {
  ...
  list(
    ...
    analysis = list(
      primary_population = "submitted_only", denominator = "valid_ratings", quantile_type = 7,
      consensus = list(
        agree_values = 7:9, disagree_values = 1:3, min_valid_n = 10,
        `in` = list(agree = b("gte", .7), disagree = b("lt", .15)), out = list(agree = b("lt", .15), disagree = b("gte", .7)),
        group_policy = "all_required_groups"
      ),
      stability = list(
        enabled = TRUE, comparable_versions_only = TRUE,
        metrics = c("median_absolute_change", "proportion_unchanged"), decision_threshold = NULL
      )
    ),
    feedback = list(
      own_previous_rating = TRUE, distribution = TRUE, median = TRUE, iqr = TRUE, valid_n = TRUE,
...
config_scales <- function(p) {
  lapply(p$instrument$scales, function(s) {
...
check_protocol <- function(p) {
  ...
  ensure(is.character(groups) && length(groups) > 0 && all(!is.na(groups) & nzchar(trimws(groups))) && !anyDuplicated(groups) && !"overall" %in% groups, "panel.groups")
  ...
  ensure(is.list(p$instrument$scales) && length(p$instrument$scales) > 0 &&
  ...
  ensure(is.logical(p$analysis$stability$enabled) && length(p$analysis$stability$enabled) == 1 &&
    !is.na(p$analysis$stability$enabled), "stability.enabled")
  ...
  ensure(all(vapply(p$feedback[c("distribution", "median", "iqr", "valid_n")], isTRUE, logical(1))), "feedback.statistics_unsupported")
