...
js("document.getElementById('panel-consent_check').click();document.getElementById('panel-consent').click()");Sys.sleep(.5)
...
x<-js("document.getElementById('panel-receipt').textContent")$result$value;print(x);stopifnot(length(x)==1L);stopifnot(grepl('Abgabe bestätigt:',x,fixed=TRUE))
b$close()
