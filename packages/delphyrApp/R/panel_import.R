  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$body <- shiny::renderUI({
      ...
      shiny::tags$section(
        ...
        shiny::fileInput(ns("file"), tr(l, "Paneldatei (CSV, h\u00f6chstens 1 MiB)", "Panel file (CSV, maximum 1 MiB)"), accept = ".csv"),
        shiny::selectInput(ns("schema"), tr(l, "Dateischema", "File schema"), c("1.0" = "1.0")),
        shiny::selectInput(ns("delimiter"), tr(l, "Trennzeichen", "Delimiter"), stats::setNames(c(",", ";"), c(tr(l, "Komma", "Comma"), tr(l, "Semikolon", "Semicolon")))),
        ...
        shiny::textAreaInput(ns("reason"), tr(l, "Importbegr\u00fcndung", "Import rationale"), width = "100%"),
    ...
    output$rows <- shiny::renderTable(
      {
        ...
        names(x) <- if (lang() == "en") c("Row", "Source reference", "Normalized email", "Display name", "Language", "Stakeholder group") else c("Zeile", "Quellenreferenz", "Normalisierte E-Mail", "Anzeigename", "Sprache", "Interessengruppe")
        x
      },
      striped = TRUE
    )
